...
from data_api.providers.fixture_statistics_provider import fetch_fixture_statistics

def collect_fixture_statistics(league_id: int, season: int, max_items: int = 10) -> dict:
    raw_root = season_root(league_id, season)
    fixture_ids = read_json(raw_root / "catalog" / "fixture_ids.json").get("fixture_ids", [])
    out_dir = raw_root / "statistics"
    state_file = state_root(league_id, season) / "resume_statistics.state.json"
    ...
    for fx in pending:
        data = fetch_fixture_statistics(fx)
        write_json(out_dir / f"fixture_{fx}.json", data)
        done.add(fx)
        state["done"] = sorted(done)
        state["last_success_item"] = fx
        save_state(state_file, state)
    return {"statistics_collected_now": len(pending), "done_total": len(done)}
