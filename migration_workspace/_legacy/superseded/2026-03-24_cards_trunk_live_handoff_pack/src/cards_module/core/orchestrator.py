...
def run_cards_pipeline(league_id: int, season: int) -> Dict:
    ...
    payload = {
        ...
        'data_quality_flag': 'clean' if engine_input.get('sample_size', 0) >= 2 else 'partial',
        'module_rank_internal': 1,
        'module_specific_payload': {
            'provider_official': 'data_api.services.fixtures_service + data_api.services.events_service',
            'official_object_consumed': 'fixtures + fixture_events',
            ...
            'eligibility_gate': 'avg_cards>=3.5 and sample_size>=2 and top_fixture_cards>=4',
        },
    }
    return payload
