...
def build(summary: dict):
    ...
    readiness = summary.get('readiness_level', 'rehearsal')
    cta_state = 'blocked' if blocked else ('ready_live' if readiness == 'real_ready' else 'ready_partial' if readiness == 'partial_real' else 'ready_rehearsal')
    return {
        ...
        'target_intent': summary.get('target_intent', 'staging'),
