...
def adapt_protected_input(payload):
    required_top = ["fixture_id","league_id","season","home_team_id","away_team_id","provider_name","provider_source","protected_match_context"]
    ...
    required_ctx = ["lambda_league","GPI_M","DRI_M","OPI_M","NRI_M","AttackAdj","DefenseAdj","PaceAdj","RiskAdj"]
    ...
    return {
        ...
        "runtime_profile": payload.get("runtime_profile","mock_adapter_run"),
...
def emit_output(adapted):
    if adapted["runtime_state"] == "hard_fail":
        return {
            ...
            "module_version": "mock_adapter_v1",
...
def main():
    ...
    print("Under/Over Jogo - aproximacao controlada mockada")
    print()
    for path in files:
        raw = load_json(path)
