...
from provider_bridge import FileCentralProvider, ProviderBridge, resolve_official_sample_path
from input_adapter import build_v12_input
from contract_output import build_market_pick_v11

MODULE_VERSION = 'v12.10-official-provider-smoke'
FIXTURE_ID = 878317
...
def run_smoke(sample_path: str | None = None) -> dict:
    pack_root = Path(__file__).resolve().parent.parent
    resolved = Path(sample_path).resolve() if sample_path else resolve_official_sample_path(pack_root)
    provider_mode = 'official_file' if 'data_api' in resolved.parts else 'staging_fallback'
    provider = FileCentralProvider(sample_path=str(resolved), provider_mode=provider_mode)
    bridge = ProviderBridge(provider=provider)
    central_payload = bridge.get_central_payload(FIXTURE_ID)
    ...
    return {
        'provider_meta': central_payload.get('_provider_meta', {}),
        'provider_sample_path': str(resolved),
        'adapter_meta': v12_input['adapter_meta'],
        'outputs': outputs,
    }
