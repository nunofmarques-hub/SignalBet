...
from providers.official_live_provider import OfficialProviderUnavailable, load_cards_context
...
def run_cards_pipeline(league_id: int, season: int) -> Dict:
    ...
    is_candidate = eligibility['eligibility']
    payload = {
        ...
        'line': 3.5,
        'odds': 1.80,
        'eligibility': is_candidate,
        'score_raw': scoring['score_raw'],
        'confidence_raw': scoring['confidence_raw'],
        'risk_raw': scoring['risk_raw'],
        'edge_raw': scoring['edge_raw'],
        'rationale_summary': 'Leitura oficial do trunk baseada em fixtures e card events por fixture, com shortlist pelo maior contexto disciplinar observado.',
        'main_drivers': [
        ...
        'pack_status': 'candidate' if is_candidate else 'rejected',
    }
    return payload
