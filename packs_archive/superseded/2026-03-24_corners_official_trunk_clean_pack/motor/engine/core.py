def eval_case(adapted):
    ...
    over_raw=(h['corners_for_avg']*10*0.18+a['corners_for_avg']*10*0.16+h['shots_avg']*4*0.10+a['shots_avg']*4*0.08+h['final_third_pressure']*0.10+a['final_third_pressure']*0.08+h['territorial_proxy']*0.06+a['territorial_proxy']*0.04+(h['hit_rate_over_9_5']*100)*0.08+(a['hit_rate_over_9_5']*100)*0.06+c['expected_pace_shape']*0.04+c['expected_game_state_volatility']*0.02)
    ...
    final=score_pre - (6 if 'rotação provável' in alerts else 0) - (8 if 'perfil tático instável' in alerts else 0) - (25 if exclusions else 0)
    ...
    conf='Alta' if (final*0.7+line_strength*0.3-(6 if alerts else 0))>=80 else ('Média' if (final*0.7+line_strength*0.3-(6 if alerts else 0))>=64 else 'Baixa')
    ...
    return {'fixture_id': adapted['fixture_id'],'game': f"{adapted['home_team_name']} vs {adapted['away_team_name']}",'competition': adapted['competition'],'market': f"{direction} {line} cantos jogo",'score': final,'confidence': conf,'risk': risk,'eligible': status!='rejected','candidate_status': status,'band': band,'estimated_total_corners': est,'main_drivers': ['produção combinada acima do padrão'] if direction=='Over' else ['produção combinada abaixo do ideal'],'alerts': alerts+exclusions,'team_scores': {'home': round((h['corners_for_avg']*10+h['final_third_pressure'])/2,2), 'away': round((a['corners_for_avg']*10+a['final_third_pressure'])/2,2)},'game_profile': 'Volume Alto' if est>=11.2 else ('Volume Médio/Alto' if est>=9.6 else 'Volume Médio'),'market_bias': 'Over' if est-line>=1.2 else ('Under' if est-line<=-1.2 else 'Zona Cinzenta'),'line_grid': [{'market': f"{direction} 8.5 cantos",'score': round(max(0,min(100,52+((est-8.5)*20 if direction=='Over' else (8.5-est)*20))),2)},{'market': f"{direction} 9.5 cantos",'score': round(max(0,min(100,52+((est-9.5)*20 if direction=='Over' else (9.5-est)*20))),2)},{'market': f"{direction} 10.5 cantos",'score': round(max(0,min(100,52+((est-10.5)*20 if direction=='Over' else (10.5-est)*20))),2)}],'rationale': {'selected_line': line,'corner_projection': est,'candidate_status': status,'band': band},'operational_conclusion': f"Projeção {est}, linha {line}, score {final}, confiança {conf}, risco {risk}."}
