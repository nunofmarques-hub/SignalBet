...
def run_validation(ctx: dict):
    ...
    pool = collect(ctx['run_id'], mod_results) if preflight != 'FAIL' else {'run_id': ctx['run_id'], 'candidate_count': 0, 'candidates': [], 'sources': {}}
    ...
    summary = {
        ...
        'module_feed_sources': pool.get('sources', {}),
        ...
        'final_summary': (
            ...
            f"cand={pool['candidate_count']}; gps={gps_out['status']}; banca={bank_out['status']}; exec={exec_out['status']}; mode={ctx['project_mode']}."
        )
    }
    ui = build_ui(summary)
