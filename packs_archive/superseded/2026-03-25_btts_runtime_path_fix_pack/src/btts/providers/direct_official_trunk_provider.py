...
class BTTSDirectOfficialTrunkProvider:
    """Provider principal do BTTS.

    Caminho principal:
    - tenta ler diretamente do Data_API_Official_Trunk_v1

    Caminho secundário:
    - usa fallback serializado apenas como apoio técnico de teste
    """
    ...
    def __init__(self, fallback_path):
        ...
        if candidate.exists():
            self.fallback_path = candidate
            return

        # hardening: recalcular relativamente à raiz real do pack
        # direct_official_trunk_provider.py está em <pack_root>/src/btts/providers/
        pack_root = Path(__file__).resolve().parents[3]
        self.fallback_path = pack_root / "support" / "fallback" / "fallback_official_trunk_input_btts.json"
    ...
    def build_official_input(self, league_id: int, season: int, fixture_index: int = 0):
        if self._real_trunk_available():
            ...
            payload["data_api_context"]["provider"] = "Data_API_Official_Trunk_v1"
            payload["data_api_context"]["provider_version"] = "official_trunk.v1"
        ...
        fallback["data_api_context"]["source_profile"] = "fallback_serialized_for_test"
        return fallback
