...
def _readiness_level(preflight_status: str, project_mode: str, coverage: float) -> str:
    if preflight_status == 'FAIL':
        return 'blocked'
    if project_mode != 'demo' and coverage >= 0.75:
        return 'real_ready'
    if project_mode != 'demo' and coverage > 0:
        return 'partial_real'
    return 'rehearsal'
...
def run_validation(ctx: dict):
    ...
    data_checks = data.run(str(project_root))
    checks += data_checks; pipeline_steps.append({'step':'data_api_readiness_check','status':'PASS' if not _blocking(data_checks) else 'FAIL'})
    ...
    coverage = round((feed_stats.get('project',0) / max(1, len(eligible))), 3)
    ...
    readiness_level = _readiness_level(preflight_status, ctx['project_mode'], coverage)
    summary = {
        'run_id': ctx['run_id'],
        ...
        'target_intent': ctx.get('target_intent', 'staging'),
        'preflight_status': preflight_status,
        'official_trunk_status': 'PASS' if not _blocking(data_checks) else 'FAIL',
        ...
        'final_summary': f"{ctx['run_profile']} concluído em modo {run_mode}. readiness={readiness_level}. Elegíveis={len(eligible)}, corridos={len(ran)}, failed={len(failed)}, candidatos={pool['candidate_count']}, aprovadas={len(bank_out['approved'])}, coverage_real={round(coverage * 100, 1)}%."
    }
